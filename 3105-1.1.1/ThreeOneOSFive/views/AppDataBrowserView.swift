...
struct AppDataBrowserView: View {
    ...
    private func reload() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            // These two scans are independent and are often the slowest part of
            // opening App Data. Run them together; MCM work starts after the
            // lightweight inputs are ready. The discovery cache above also
            // makes repeated refreshes substantially cheaper.
            let group = DispatchGroup()
            var bundleMetadata: [String: ApplicationBundleMetadata] = [:]
            var filesystemApps: [InstalledApp] = []

            group.enter()
            DispatchQueue.global(qos: .userInitiated).async {
                bundleMetadata = ContainerStore.applicationBundleMetadataCatalog()
                group.leave()
            }

            group.enter()
            DispatchQueue.global(qos: .utility).async {
                filesystemApps = ContainerStore.containersFromFilesystem()
                group.leave()
            }
