...
// MARK: - Short-lived discovery cache
// Container UUIDs are intentionally NOT cached because they can change after
// reinstall/restore. Only expensive discovery inputs are cached briefly.
enum BrowserDiscoveryCache {
    private static let lock = NSLock()
    private static let metadataTTL: TimeInterval = 30
    private static let identifierTTL: TimeInterval = 10
    private static let launchServicesTTL: TimeInterval = 10

    private static var metadataValue: [String: ApplicationBundleMetadata]?
    private static var metadataDate: Date = .distantPast
    private static var identifierValue: [String]?
    private static var identifierDate: Date = .distantPast
    private static var launchServicesValue: [String]?
    private static var launchServicesDate: Date = .distantPast

    static func cachedMetadata() -> [String: ApplicationBundleMetadata]? {
        lock.lock(); defer { lock.unlock() }
        guard let value = metadataValue, Date().timeIntervalSince(metadataDate) < metadataTTL else { return nil }
        return value
    }

    static func storeMetadata(_ value: [String: ApplicationBundleMetadata]) {
        lock.lock(); metadataValue = value; metadataDate = Date(); lock.unlock()
    }

    static func cachedIdentifiers() -> [String]? {
        lock.lock(); defer { lock.unlock() }
        guard let value = identifierValue, Date().timeIntervalSince(identifierDate) < identifierTTL else { return nil }
        return value
    }

    static func storeIdentifiers(_ value: [String]) {
        lock.lock(); identifierValue = value; identifierDate = Date(); lock.unlock()
    }

    static func cachedLaunchServices() -> [String]? {
        lock.lock(); defer { lock.unlock() }
        guard let value = launchServicesValue, Date().timeIntervalSince(launchServicesDate) < launchServicesTTL else { return nil }
        return value
    }

    static func storeLaunchServices(_ value: [String]) {
        lock.lock(); launchServicesValue = value; launchServicesDate = Date(); lock.unlock()
    }

    static func invalidate() {
        lock.lock()
        metadataValue = nil; identifierValue = nil; launchServicesValue = nil
        metadataDate = .distantPast; identifierDate = .distantPast; launchServicesDate = .distantPast
        lock.unlock()
    }
}
